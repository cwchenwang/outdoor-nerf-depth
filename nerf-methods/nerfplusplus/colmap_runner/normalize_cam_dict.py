...
def normalize_cam_dict(in_cam_dict_file, out_cam_dict_file, target_radius=1., in_geometry_file=None, out_geometry_file=None):
    ...
    for img_name in out_cam_dict:
        ...
        out_cam_dict[img_name]['C2W'] = list(np.linalg.inv(W2C).flatten())
...
if __name__ == '__main__':
    path = '/ssd1/wangchen/data/nerfpp_preprocessed/kittiSeq02_llffdtu_2011_10_03_drive_0034_sync_170scans_densegt'
    in_cam_dict_file = os.path.join(path, 'kai_cameras.json')
    out_cam_dict_file = os.path.join(path, 'kai_cameras_norm.json')
