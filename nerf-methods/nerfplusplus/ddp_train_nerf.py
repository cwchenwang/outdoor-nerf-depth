...
from utils import img2mse, mse2psnr, img_HWC2CHW, colorize, TINY_NUMBER, colorize_np, to8b
import logging
import json
import imageio
...
depth_losses_dict = {
    ...
    'l1': depth_l1,
    'kl': depth_kl,
    'los': depth_light_of_sight,
    'nll': depth_gaussian_log_likelihood,
}
...
def log_view_to_tb(writer, global_step, log_data, gt_img, mask, depth_gt=None, depth_sup=None, prefix=''):
    rgb_im = img_HWC2CHW(torch.from_numpy(gt_img))
    writer.add_image(prefix + 'rgb_gt', rgb_im, global_step)

    if depth_gt is not None:
        depth_gt_img = img_HWC2CHW(colorize(depth_gt, cmap_name='jet', append_cbar=True,
                                        mask=mask))
        writer.add_image(prefix + 'depth_gt', depth_gt_img, global_step)
    
    if depth_sup is not None:
        depth_sup_img = img_HWC2CHW(colorize(depth_sup, cmap_name='jet', append_cbar=True,
                                        mask=mask))
        writer.add_image(prefix + 'depth_sup', depth_sup_img, global_step)
...
def ddp_train_nerf(rank, args):
    ...
    ray_samplers = load_data_split(args.datadir, args.scene, split='train',
                                   try_load_min_depth=args.load_min_depth, skip=args.trainskip, depth_sup_type=args.depth_sup_type)
    val_ray_samplers = load_data_split(args.datadir, args.scene, split='test',
                                       try_load_min_depth=args.load_min_depth, skip=args.testskip, depth_sup_type=args.depth_sup_type)
    ...
    for global_step in range(start+1, start+1+args.N_iters):
        ...
        for m in range(models['cascade_level']):
            ...
            depth_sup = ray_batch['depth_sup'].to(rank)
            if args.use_depth and 'depth_sup' in ray_batch:
                if args.depth_loss_type == 'kl':
                    depth_loss = depth_kl(ret['fg_weights'], depth_sup, fg_depth, ret['fg_dists'], args.depth_sigma * ray_samplers[i].get_depth_scale(), fg_far_depth)
                else:
                    depth_loss = depth_losses_dict[args.depth_loss_type](depth_sup, ret['depth'])
                scalars_to_log['level_{}/loss_depth'.format(m)] = depth_loss.item()
        ...
        #         log_view_to_tb(writer, global_step, log_data, gt_img=val_ray_samplers[idx].get_img(), depth_gt=val_ray_samplers[idx].get_gt_depth_img(),  depth_sup=val_ray_samplers[idx].get_sup_depth_img(), mask=None, prefix='val/')
        ...
        #         log_view_to_tb(writer, global_step, log_data, gt_img=ray_samplers[idx].get_img(), depth_gt=ray_samplers[idx].get_gt_depth_img(), depth_sup=ray_samplers[idx].get_sup_depth_img(), mask=None, prefix='train/')
        ...
        if (global_step % 50000 == 0) and global_step != 0:
            out_dir = os.path.join(args.basedir, args.expname,
                               'render_{}_{:06d}'.format('test', global_step))
            os.makedirs(out_dir, exist_ok=True)
            psnrs = []
            rmses = []
            abs_rels = []
            rmse_tot, rmse_log_tot, abs_diff, abs_rel, sq_rel = 0, 0, 0, 0, 0
            for idx in range(len(val_ray_samplers)):
                ### each process should do this; but only main process merges the results
                ret = render_single_image(rank, args.world_size, models, val_ray_samplers[idx], args.chunk_size)
                if rank == 0:    # only main process should do this
                    ...
                    if val_ray_samplers[idx].img_path is not None:
                        gt_im = val_ray_samplers[idx].get_img()
                        psnr = mse2psnr(np.mean((gt_im - im) * (gt_im - im)))
                        logger.info('{}: psnr={}'.format(fname, psnr))
                        psnrs.append(psnr)

                        error = np.abs(im - gt_im).mean(-1).astype(np.float32)
                        error = (error - error.min()) / (error.max() - error.min())
                        imageio.imwrite(os.path.join(out_dir, 'error_rgb_' + fname), (np.clip(error, 0., 1.) *255.).astype(np.uint8))
                    
                    if val_ray_samplers[idx].depth_gt_path is not None:
                        depth_gt = val_ray_samplers[idx].get_gt_depth_img()
                        current_gt_sparse = depth_gt / val_ray_samplers[idx].get_depth_scale()

                        current_pred = ret[-1]['depth'].numpy() / val_ray_samplers[idx].get_depth_scale()
                        ...
                        abs_rel_save = (abs_rel_save - abs_rel_save.min()) / (abs_rel_save.max() - abs_rel_save.min())
                        imageio.imwrite(os.path.join(out_dir, 'absrel_' + fname), (np.clip(abs_rel_save, 0., 1.) *255.).astype(np.uint8))
            ...
            if len(psnrs) > 0:
                ...
                writer.add_scalar('test_psnr', psnr_mean, global_step)
                with open(os.path.join(out_dir, f'psnr_{global_step:06d}.txt'), 'w') as f:
                    f.write('\n'.join([str(p) for p in psnrs]))
            if len(rmses) > 0:
                rmse_mean = np.mean(rmses)
                rmses.append(rmse_mean)
                with open(os.path.join(out_dir, f'rmse_{global_step:06d}.txt'), 'w') as f:
                    f.write('\n'.join([str(p) for p in rmses]))
                writer.add_scalar('test_rmse', rmse_mean, global_step)
                absrel_mean = np.mean(abs_rels)
                abs_rels.append(absrel_mean)
                with open(os.path.join(out_dir, f'absrel_{global_step:06d}.txt'), 'w') as f:
                    f.write('\n'.join([str(p) for p in abs_rels]))
                writer.add_scalar('test_absrel', absrel_mean, global_step)
...
def config_parser():
    ...
    parser.add_argument("--trainskip", type=int, default=1,
                        help='will load 1/N images from train sets for sparse inputs')
    # model size
    ...
    parser.add_argument("--use_depth", action='store_true', help='whether to use depth supervision')
    parser.add_argument("--lambda_depth", type=float, default=1.0)
    parser.add_argument("--depth_loss_type", choices=['mse', 'kl', 'los', 'l1', 'nll'], default='mse')
    parser.add_argument("--depth_sup_type", type=str, default='gt')
...
def train():
    ...
    if os.path.exists(os.path.join(args.basedir, args.expname)):
