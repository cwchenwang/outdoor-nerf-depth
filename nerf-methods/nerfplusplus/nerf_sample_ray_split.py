...
class RaySamplerSingleImage(object):
    def __init__(self, H, W, intrinsics, c2w,
        ...
        self.depth_sup_path = depth_sup_path
    ...
    def set_resolution_level(self, resolution_level):
        if resolution_level != self.resolution_level:
            ...
            if self.depth_gt_path is not None:
                self.depth_gt = np.array(imageio.imread(self.depth_gt_path).astype(np.float32)) / 256.0
                self.depth_gt = self.depth_gt.reshape((-1))
                self.depth_gt = self.depth_scale * self.depth_gt
            
            if self.depth_sup_path is not None:
                self.depth_sup = np.array(imageio.imread(self.depth_sup_path).astype(np.float32)) / 256.0
                self.depth_sup = self.depth_sup.reshape((-1))
                self.depth_sup = self.depth_scale * self.depth_sup
    ...
    def get_gt_depth_img(self):
        if self.depth_gt is not None:
            return self.depth_gt.reshape((self.H, self.W))
        else:
            return None
    
    def get_sup_depth_img(self):
        if self.depth_sup is not None:
            return self.depth_sup.reshape((self.H, self.W))
        else:
            return None
    ...
    def get_all(self):
        ...
        if self.depth_gt_path is not None:
            ret['depth_gt'] = self.depth_gt
        if self.depth_sup_path is not None:
            ret['depth_sup'] = self.depth_sup
    ...
    def random_sample(self, N_rand, center_crop=False):
        ...
        if self.depth_sup_path is not None:
            ...
            ret['depth_gt'] = depth_gt
        if self.depth_sup_path is not None:
            depth_sup = self.depth_sup[select_inds]
            ret['depth_sup'] = depth_sup
