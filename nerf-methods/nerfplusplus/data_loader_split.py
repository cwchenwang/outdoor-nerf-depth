...
def load_data_split(basedir, scene, split, skip=1, try_load_min_depth=True, only_img_files=False, depth_sup_type='gt'):
    ...
    # depth files
    depth_gt_files = find_files('{}/depth'.format(split_dir), exts=['*.png', '*.jpg'])
    if len(depth_gt_files) > 0:
        logger.info('raw depth_gt_files: {}'.format(len(depth_gt_files)))
        depth_gt_files = depth_gt_files[::skip]
        assert(len(depth_gt_files) == cam_cnt)

        depth_scale = float(open(os.path.join(basedir, scene, 'scale')).readlines()[0].strip())
    else:
        depth_files = [None, ] * cam_cnt
        depth_scale = None
    
    suffix = '_' + depth_sup_type if depth_sup_type != 'gt' else ''
    depth_sup_files = find_files('{}/depth{}'.format(split_dir, suffix), exts=['*.png', '*.jpg'])
    if len(depth_sup_files) > 0:
        logger.info('raw depth_sup_files: {}'.format(len(depth_sup_files)))
        depth_sup_files = depth_sup_files[::skip]
        assert(len(depth_sup_files) == cam_cnt)
    else:
        depth_sup_files = [None, ] * cam_cnt
    ...
    for i in range(cam_cnt):
        ...
        ray_samplers.append(RaySamplerSingleImage(H=H, W=W, intrinsics=intrinsics, c2w=pose,
                                                  ...
                                                  depth_sup_path=depth_sup_files[i],
