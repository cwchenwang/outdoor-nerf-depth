...
class Config:
  ...
  sample_every: int = 1 # Sample every Nth frame for sparse input views.
  ...
  depth_keep_ratio: float = 0.0 # mask ratio (only used when compute_depth_metrics=True)
  depth_crop_range: float = 0.0 # values large than the range will be set to invalid
  depth_loss_type: str = 'mse' # depth type, should choose from 'mse', 'nll' etc.
  depth_sup_type: str = 'gt' # depth supervision depth, should choose from 'gt', 'stereo' etc.
