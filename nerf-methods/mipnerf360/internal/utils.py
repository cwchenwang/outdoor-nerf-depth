...
class Batch:
  ...
  disps_gt: Optional[_Array] = None
  disps_sup: Optional[_Array] = None
...
def save_rgb_error(img, img_gt, pth):
  with open_file(pth, 'wb') as f:
    error = np.abs(img - img_gt).mean(-1).astype(np.float32)
    error = (error - error.min()) / (error.max() - error.min())
    Image.fromarray(
      (np.clip(error, 0., 1.) *255.).astype(np.uint8)).save(
        f, 'PNG')

def save_normalized(data, pth):
  with open_file(pth, 'wb') as f:
    data = (data - data.min()) / (data.max() - data.min())
    Image.fromarray(
      (np.clip(data, 0., 1.) *255.).astype(np.uint8)).save(
        f, 'PNG')

def save_npy(data, pth):
  """Save an image (probably RGB) in [0, 1] to disk as a uint8 PNG."""
  with open_file(pth, 'wb') as f:
    np.save(f, data)
