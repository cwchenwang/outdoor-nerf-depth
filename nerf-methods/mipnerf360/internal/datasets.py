...
class LLFF(Dataset):
  ...
  def _load_renderings(self, config):
    ...
    if config.rawnerf_mode:
      ...
      depth_dir = os.path.join(self.data_dir, 'depths_gt' + image_dir_suffix)
      suffix = '_' + config.depth_sup_type
      depth_sup_dir = os.path.join(self.data_dir, 'depths' + image_dir_suffix + suffix)
      ...
      if self._load_disps:
        ...
        if config.depth_keep_ratio > 0:
          # if config.depth_sup_type == 'gt':
          full_ratio = np.count_nonzero(depths_sup>0) / np.prod(depths_sup.shape)
          assert(config.depth_keep_ratio < full_ratio)
          ratio = config.depth_keep_ratio / full_ratio # sample ratio in total image
          np.random.seed(0)
          mask = np.bitwise_and(depths_sup > 0, np.random.uniform(0, 1, depths_sup.shape) < ratio)
          depths_sup[~mask] = -256.
        # depths_sup[depths_sup > 0] = np.random.rand(*depths_sup.shape).astype(depths_sup.dtype)[depths_sup>0]
        # depths_sup_temp = depths_sup[depths_sup>0]
        # depths_sup_temp[:(depths_sup_temp.shape[0])//10*9] = -1
        # depths_sup[depths_sup>0] = depths_sup_temp
    ...
    if config.forward_facing:
      ...
      scale = np.sqrt((transform[:3, :3]@transform[:3, :3].T)[0, 0])
      if config.auto_adjust_near_far is True:
        config.near *= scale
        config.far *= scale
        self.far = config.far
        self.near = config.near
        config.auto_adjust_near_far = False
    ...
    if config.sample_every >= 1:
      test_indices = [i for i in range(9, images.shape[0], 10)]
      trains = list(set(range(images.shape[0])) - set(test_indices))
      # ids = [trains]
      train_indices = [trains[i] for i in range(0, len(trains), config.sample_every)]
      split_indices = {
        utils.DataSplit.TEST: np.array(test_indices),
        utils.DataSplit.TRAIN: np.array(train_indices)
      }
