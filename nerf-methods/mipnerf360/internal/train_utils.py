...
import optax
from internal import depth_loss
...
def compute_data_loss(batch, renderings, ray_history, rays, config):
  ...
  for i, rendering in enumerate(renderings):
    ...
    if config.compute_disp_metrics:
      if config.depth_loss_type == 'mse':
        ...
        disp_mask = (batch.disps_sup > 0)
        # disp_mse = (disp_mask * (disp - (1/(1+disp_mask * batch.disps_sup)))**2).mean()
        disp_mse = ((disp_mask * rendering['distance_mean'] - disp_mask * batch.disps_sup) ** 2).mean()
        # stats['disparity_mses'].append(disp_mse)
        depth_losses.append(disp_mse)
      elif config.depth_loss_type == 'l1':
        disp_mask = (batch.disps_sup > 0)
        disp_l1 = jnp.abs(disp_mask * rendering['distance_mean'] - disp_mask * batch.disps_sup).mean()
        depth_losses.append(disp_l1)
      else:
        pseudo_gt_depth = batch.disps_sup
        predict_depth = rendering['distance_mean']
        sigma = config.depth_sigma * config.depth_scale
        weights = ray_history[i]['weights']
        tdist = ray_history[i]['tdist']
        depth_loss_val = depth_loss.depth_loss(weights, tdist, pseudo_gt_depth, predict_depth, sigma, rays.directions, config.depth_loss_type)
  ...
  if config.compute_disp_metrics:
    depth_losses = jnp.array(depth_losses)
    loss = (
        config.data_coarse_loss_mult * (jnp.sum(data_losses[:-1]) + config.lambda_depth*jnp.sum(depth_losses[:-1]) ) +
        config.data_loss_mult * ( data_losses[-1] + config.lambda_depth*depth_losses[-1])
    )
    stats['loss_disp_mse'] = config.lambda_depth*jnp.sum(depth_losses)
  else:
    loss = (
...
def create_train_step(model: models.Model,
  ...
  def train_step(
    ...
    def loss_fn(variables):
      ...
      data_loss, stats = compute_data_loss(batch, renderings, ray_history, rays, config)
      losses['data'] = data_loss

      if 'loss_disp_mse' in stats.keys():
        losses['disp_mse'] = stats['loss_disp_mse']
