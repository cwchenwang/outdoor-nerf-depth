...
def main(unused_argv):
  ...
  out_dir = path.join(config.checkpoint_dir,
                      'path_renders' if config.render_path else f'test_eval_preds_{config.eval_suffix}')
  if config.restore_step is not None:
    out_dir += '_' + str(config.restore_step)
  ...
  while True:
    state = checkpoints.restore_checkpoint(config.checkpoint_dir, state, step=config.restore_step)
    step = int(state.step)
    if step <= last_step:
          ...
          for tag in ['mean', 'median']:
            ...
            if key in rendering:
              ...
              metric[f'disparity_{tag}_mse'] = float(
                  ((disparity - batch.disps_gt)**2).mean())
    ...
    if (config.eval_save_output and (not config.render_path) and
      ...
      for name in metrics[0]:
        with utils.open_file(path_fn(f'metric_{name}_{step}.txt'), 'w') as f:
          f.write('\n'.join([str(m[name]) for m in metrics]))
      for name in metrics_cc[0]:
