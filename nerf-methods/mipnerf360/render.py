...
def main(unused_argv):
  ...
  for idx in range(dataset.size):
    ...
    ray_batch = dataset.generate_ray_batch(idx)
    rays = ray_batch.rays
    depth_gt = ray_batch.disps_gt
    train_frac = 1.
    ...
    cap = 80
    current_gt_sparse = depth_gt / config.depth_scale
    ...
    abs_rel = np.abs(valid_gt - valid_pred)
    abs_rel_save[valid] = abs_rel

    Image.fromarray((current_pred.clip(1e-3, 80) * 256.0).astype(np.uint16)).save(
      path_fn(f'depth_{idx_str}.png')
    )
    save_fn(
      utils.save_npy, abs_rel_save, path_fn(f'absrel_{idx_str}.npy')
    )
