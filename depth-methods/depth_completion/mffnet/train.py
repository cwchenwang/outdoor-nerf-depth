...
def train(epoch):
    ...
    for batch_idx, (rgb, lidar, depth) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        loss_net = net(rgb, lidar, depth, 'train')#.mean()
#         loss = net.compute_loss(depth)
        #.mean()
        loss = criterion(loss_net, depth).mean()
#         print(loss)
        loss.backward()
        optimizer.step()
